...
<img 
src={artistId ? artistPath?.artwork?.url.replace('{w}','500').replace('{h}', '500') : songData?.images?.coverart}
alt='lyrics'
className="sm:w-48 w-28 sm:h-48 h-28 rounded-full object-cover
border-2 shadow-xl shadow-[#161616]" 
/>
<div className="ml-3">
<h2 className="font-bold sm:text-3xl text-xl">{artistId ? artistPath?.name : songData?.title}</h2>
{!artistId && (
        ...
        <p className="text-base text-gray-400 mt-2">{artistId ? artistPath?.genreNames[0] 
